	...
	URL: {
		...
		BEESWORM: function(genename, cancertype) {
			...
			if($("#simple_nonmalignant").is(':checked') && $("#simple_tumor").is(':checked')) {
				return beesworm.URL.BASE_URL() + genename + "/" + cancertype + "/" + $("#dataType").val() + "/" + value + "/bycancer";
			} else if($("#simple_tumor").is(':checked')) {
				return beesworm.URL.BASE_URL() + genename + "/" + cancertype + "/" + $("#dataType").val() + "/" + value + "/bycancer/tumor";
			} else if($("#simple_nonmalignant").is(':checked')) {
				return beesworm.URL.BASE_URL() + genename + "/" + cancertype + "/" + $("#dataType").val() + "/" + value + "/bycancer/nonmalignant";
			} else {
				return beesworm.URL.BASE_URL() + genename + "/" + cancertype + "/" + $("#dataType").val() + "/" + value + "/bycancer";
			}
		}
		...
		draw: function(genename, cancertype, data, datatag, cutoff, beesStyle, is_reRender, is_downloadConfigured) {
			...
			if($("#simple_nonmalignant").is(':checked') && $("#simple_tumor").is(':checked')) {
				glist.forEach(function(element) {
					glist_ex.push(element + "_N");
					glist_ex.push(element + "_T");
				}, this);
			} else if($("#simple_tumor").is(':checked')) {
				glist.forEach(function(element) {
					glist_ex.push(element + "_T");
			...
			option = {
				title: {
					text: cancertype.toUpperCase() + title,
					x: 'left',
					y: 0
				},
				grid: [{
			...
			myChart.on("dblclick", function(param) {
				...
				if(param.type == 'hover') {
					...
					if((Math.round(param.data[0]) - 1) % 3 == 0) {
						$("#note").append('<div class="alert alert-info alert-dismissible" role="alert" style="padding:0;margin:0;margin-top:1px;font-weight:light;font-size:0.8em;">' +
						...
						$("#note").append('<div class="alert alert-danger alert-dismissible" role="alert" style="padding:0;margin:0;margin-top:1px;font-weight:light;font-size:0.8em;">' +
							'<button type="button" style="padding:0;margin:0;" class="close" data-dismiss="alert">' +
							'<span aria-hidden="true">&times;</span><span class="sr-only">Close</span></button>' +
							'<strong style="padding:0;margin:0;font-weight:light;font-size:0.8em;">' + param.data[2] + ' </strong> ' + param.data[1] +
							'</div>');
